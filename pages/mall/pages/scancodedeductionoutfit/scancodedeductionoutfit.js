...
Page({
          ...
          if (res.code) {
            ...
            wx.request({
              ...
              success: function (res) {
                ...
                that.selectMallPlatformTotaAmountData();
    ...
    itemCheck(){
        var rule_array=this.data.rule_array;
        if(rule_array.length<=0){
          return;
        }
        for(var index=0;index<rule_array.length;index++){
            var item=rule_array[index];
            if(item.name.includes('试衣会员')){
              this.setData({
                id:item.id,
    ...
    insertMallMemberRechargeRecord(e){
      ...
      var recharge_amount=Number(this.data.recharge_amount);
    ...
    recharge_record_wx_pay: function wechat_pay(recharge_record_item) {
      ...
      var total_amount=Number(this.data.recharge_amount);
    ...
    onShow: function () {
       //查询总金额
       //this.selectMallPlatformTotaAmountData();
    ...
    selectMallPlatformTotaAmountData(){
      var that=this;
      ...
      wx.request({
        url: app.globalData.selectMallPlatformTotaAmountData,
        ...
        success: function (res) {
            if(res.data.code==1000){
                  var mall_member_total_recharge_amount=res.data.data.mall_member_total_recharge_amount;
                  mall_member_total_recharge_amount=Number(mall_member_total_recharge_amount).toFixed(2);

                  var total_color_global_voucher=res.data.data.total_color_global_voucher;
                  total_color_global_voucher=Number(total_color_global_voucher).toFixed(2);

                  that.setData({
                    mall_member_total_recharge_amount:mall_member_total_recharge_amount,
                    total_color_global_voucher:total_color_global_voucher,
                  })
