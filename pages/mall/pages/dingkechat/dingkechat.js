...
Page({
  ...
  onLoad(options) {
    ...
    // console.log(JSON.parse(options.otherInfo))
    var that=this;
    this.setData({
      otherInfo: JSON.parse(options.otherInfo),
      meInfo: JSON.parse(options.meInfo)
    })
  ...
  getOldData(){
    ...
    if(recordList&&recordList.length>0){
        var last_index=0;
        last_max_id=recordList[0].msg_not_read_id;
    }else{
        recordList=[];
    }
    ...
    wx.request({
        url: app.globalData.selectDingMsgRecord_Old,
        //url: 'http://localhost:8080/evaluation_war/mall/selectMallCsrMsgRecord_Old',
        method: 'POST',
        data: {
          last_max_id: last_max_id,
          page_size: that.data.pageSize,
          sender_code: otherInfo.customer_code,
          receiver_code: meInfo.customer_code,
          //receiver_code: 10246,
          is_read:1,
        },
        success: res => {
          ...
          if (res.data.code == 1000) {
            ...
            if(resList&&resList.length>0){
                ...
                var current_page_id="page_"+resList[resList.length-1].msg_not_read_id;
                setTimeout(function(){
                    that.setData({
                        pageId:current_page_id,
                      })
                  },300)
                  that.selectDingMsgRecordDingKeFromChat()
            }else{
                setTimeout(function(){
                    that.setData({
                        pageId:"s2",
  ...
  getNewData(){
    ...
    if(recordList&&recordList.length>0){
        ...
        last_max_id=recordList[last_index].msg_not_read_id+1;
    }else{
        recordList=[];
    }
    ...
    wx.request({
         url: app.globalData.selectDingMsgRecord_New,
            ...
            if(resList&&resList.length>0){
                ...
                setTimeout(function(){
                    that.setData({
                      ...
                      if(that.data.pageSize==resList.length){
                        //如果还有数据，则继续获取
                        that.getNewData();
                      }
                  },300)
  ...
  sendMsgExec(msg_type,msg,duration){
    ...
    var data={
      ...
      duration:duration,
      msg: msg,
      sender_code: meInfo.customer_code,
      receiver_code: otherInfo.customer_code,
      receiver_codes:'',
      lstDingMsgCard:[],
    };
    wx.showLoading({
  ...
  sendRabbitMqMsg(){
    ...
    var sender_id=meInfo.customer_id;
    var receiver_id=otherInfo.customer_id;
    var receiver_code=otherInfo.customer_code;
    ...
    var data={
        task_queue_names:app.globalData.hj + "_qgqc_" + receiver_code,
        msg:msgJson,
    }
    wx.request({
  ...
  selectDingMsgRecordDingKeFromChat(){
    try {
        ...
        if(prePage){
            if(prePage.route){
                if(prePage.route.indexOf('pages/dingke/dingke')>=0){
                  console.log("查找到聊天列表界面")

                  prePage.selectDingMsgRecordDingKeFromChat();
                }
            }
        }
    } catch (error) {
  ...
  toPage(e){
    ...
    var item=e.currentTarget.dataset.item;
    console.log(item);
    if(item&&item.lstDingMsgCard&&item.lstDingMsgCard.length>0){
      var cardObj=item.lstDingMsgCard[0];
        var param= cardObj.page_param;
        var url=cardObj.page_url;
        if(param!=''){
          url+="?"+param;
        }
        wx.navigateTo({
